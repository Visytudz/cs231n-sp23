...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_k = np.array([sim(z_k, out[i]) for i in range(2 * N) if i != k])
        sim_k_n = np.array([sim(z_k_N, out[i]) for i in range(2 * N) if i != k + N])
        loss_k = -(np.exp(sim(z_k, z_k_N) / tau) / np.exp(sim_k / tau).sum()).log()
        loss_k_n = -(np.exp(sim(z_k_N, z_k) / tau) / np.exp(sim_k_n / tau).sum()).log()
...
def compute_sim_matrix(out):
    ...
    out = out / torch.linalg.norm(out, axis=1, keepdims=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device="cuda"):
    ...
    # sim_pairs = sim_positive_pairs(out_left, out_right).repeat(2, 1)
