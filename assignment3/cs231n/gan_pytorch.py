...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_fake = bce_loss(
        logits_fake, torch.zeros_like(logits_fake).squeeze().type(dtype)
    )
    loss_real = bce_loss(
        logits_real, torch.ones_like(logits_real).squeeze().type(dtype)
    )
...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.ones_like(logits_fake).squeeze().type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = (scores_real - 1) ** 2 * 0.5
    loss_fake = scores_fake**2 * 0.5
