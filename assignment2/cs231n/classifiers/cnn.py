...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out, cache_hidden = affine_relu_forward(out, W2, b2)
        scores, cache_out = affine_forward(out, W3, b3)
        ...
        loss += (
            self.reg
            * 0.5
            * np.sum([np.sum(W**2) for key, W in self.params.items() if "W" in key])
        )

        dx, grads["W3"], grads["b3"] = affine_backward(dx, cache_out)
        dx, grads["W2"], grads["b2"] = affine_relu_backward(dx, cache_hidden)
