...
def rmsprop(w, dw, config=None):
    ...
    lr, dr, eps, cache = (config[key] for key in keys)

    config["cache"] = cache = cache * dr + dw**2 * (1 - dr)
...
def adam(w, dw, config=None):
    ...
    lr, beta1, beta2, eps, m, v, t = (config[key] for key in keys)
